...
int main(int argc, char *argv[]) {
	...
	if (argc < 2) {
		cout << "Please give a LinkMap file path.\n[ex. ./LinkMapAnalyzer "
				"~/Library/xx-LinkMap-normal-x86_64.txt]"
			 << endl;
		return 0;
	}
	...
	const string inputFileName =
		"/Users/guomiaoyou/Documents/github/iOS-LinkMapAnalyzer/iOS-LinkMapAnalyzer/in.txt";
	ifstream inFile(inputFileName);
	//输出 重定向
	freopen(
		"/Users/guomiaoyou/Documents/github/iOS-LinkMapAnalyzer/iOS-LinkMapAnalyzer/out.txt",
		"w",
		stdout);
#endif
	...
	while (inFile.getline(buf, BUFMAXLEN - 1, '\n')) {
		...
		switch (type) {
			...
			case linkMapSections: {
				...
				analyazer->SymoblCountSize =
					(_int64 *)malloc(sizeof(_int64) * analyazer->objectFiles.size() + 1);
				memset(
					analyazer->SymoblCountSize,
					0,
					sizeof(_int64) * analyazer->objectFiles.size() + 1);
				break;
			}
